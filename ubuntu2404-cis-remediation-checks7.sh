#!/usr/bin/env bash
# Ubuntu 24.04 CIS remediation tool generated from 01-checks-7-.csv.
# Scope: all 94 failed controls in that export. Run with --audit first.
...
# Exact manifest of all failed scanner IDs in 01-checks-7-.csv.
CONTROL_IDS=(
  ...
  35543 35545 35552 35589 35591 35600 35604 35605 35606 35607 35623 35624
  35626 35627 35629 35631 35632 35633 35634
  35635 35636 35637 35638 35639 35640 35644 35646 35647 35652 35654 35657
  35664 35672 35673 35675 35687 35688 35689 35690 35694 35695 35698 35703
  35708 35710 35711 35714 35715 35720 35721 35732 35734 35735 35736 35737
  35738 35739 35742 35744 35745 35746 35747 35748 35755 35760 35765 35766
  35767 35768 35770
)
...
remediate_packages_time_cron() {
  log "Controls 35552, 35589, 35591, 35600: GDM, selected time service, and crontab access"
  ...
  systemctl list-unit-files chrony.service >/dev/null 2>&1 && mask_now chrony.service || true
  apt_purge_installed chrony
  enable_now systemd-timesyncd.service
  log "systemd-timesyncd is selected. The chrony-specific control 35591 should become not applicable."
...
remediate_network_firewall() {
  log "Controls 35623-35639: select one firewall implementation"
...
remediate_ssh_sudo() {
  log "Controls 35640, 35644, 35646, 35647, 35652, 35654, 35657, 35664"
...
remediate_pam_passwords() {
  log "Controls 35672, 35673, 35675, 35687-35690: PAM modules and password history"
  apt_install libpam-modules libpam-runtime
  if [[ "$APPLY_PAM" != "1" ]]; then
    manual "PAM profile activation is guarded. Set APPLY_PAM=1 only with console recovery after testing local and directory-backed authentication."
    return
  fi
...
Password:
 requisite pam_pwhistory.so remember=24 enforce_for_root try_first_pass use_authtok"
  if [[ "$MODE" == "apply" ]]; then
    ...
    local profile
    for profile in unix faillock faillock_notify pwhistory; do
  ...
  else log "PLAN enable unix, faillock, faillock_notify, and pwhistory with pam-auth-update"; fi
}

remediate_accounts_shell() {
  log "Controls 35694, 35695, 35698, 35703: aging, inactivity, and root umask"
  ...
  append_managed_block /root/.profile ROOT_UMASK "umask 027"
  append_managed_block /root/.bashrc ROOT_UMASK "umask 027"
}

remediate_logging() {
  log "Controls 35708, 35710, 35711, 35714, 35715, 35720, 35721: journal and rsyslog configuration"
...
*.* action(type=\"omfwd\" target=\"$REMOTE_LOG_HOST\" port=\"$REMOTE_LOG_PORT\" protocol=\"tcp\" StreamDriver=\"gtls\" StreamDriverMode=\"1\" StreamDriverAuthMode=\"x509/name\" StreamDriverPermittedPeers=\"$REMOTE_LOG_HOST\" action.resumeRetryCount=\"-1\" queue.type=\"linkedList\" queue.filename=\"cis_fwd\")"
  ...
  if [[ "$MODE" == "apply" ]]; then
    ...
    log "PLAN validate rsyslog and reload logging services"
  fi
}
...
remediate_audit() {
  log "Controls 35732, 35734-35739, 35742, 35744-35748, 35755, 35760: audit rules and tool integrity"
  apt_install auditd audispd-plugins aide acl kmod
...
validate_state() {
  log "Post-change validation snapshot"
  apparmor_status 2>/dev/null || true
  findmnt -rno TARGET,OPTIONS /tmp /home /var /var/tmp /var/log /var/log/audit 2>/dev/null || true
  systemctl is-active systemd-timesyncd.service 2>/dev/null || true
  systemctl is-active ufw.service ssh.service systemd-journal-upload.service rsyslog.service auditd.service 2>/dev/null || true
  command -v ufw >/dev/null 2>&1 && ufw status verbose 2>/dev/null || true
  command -v sshd >/dev/null 2>&1 && sshd -t 2>/dev/null || true
  command -v auditctl >/dev/null 2>&1 && auditctl -l 2>/dev/null || true
  stat -c '%U:%G %a %n' /etc/shadow /etc/shadow- /etc/gshadow /etc/gshadow- /etc/security/opasswd 2>/dev/null || true
}
...
35589: systemd-timesyncd selected and enabled; 35591 becomes N/A after chrony removal
35600: automated
...
35664: automated
35672,35673,35675,35687-35690: PAM stack activation guarded by APPLY_PAM
35694,35695,35698: defaults automated; existing users guarded by APPLY_EXISTING_USERS
35703,35708,35714,35715: automated
...
35732,35734-35739,35742,35744-35748,35755: automated audit rules and tool modes
